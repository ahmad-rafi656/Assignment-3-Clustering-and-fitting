...
def balance_data(df):
  ...
  min_length = min(len(urban_population), len(co2_emissions), len(GDP))
...
scaler = MinMaxScaler()
scaled_data = scaler.fit_transform(df[['urban_population', 'co2_emissions',
       'GDP']])
...
plot_clusters('urban_population', 'GDP')

plot_clusters('urban_population', 'co2_emissions')
...
plot_country_cluster('Madagascar', 'urban_population', 'GDP')

plot_country_cluster('Algeria', 'urban_population', 'co2_emissions')
